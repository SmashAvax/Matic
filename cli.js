...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    try {
      if (web3 == undefined) {
        ...
        contractJson = await (await fetch('build/contracts/MATICSmashnado.json')).json()
    ...
    if (web3 === undefined) {
      ...
      contractJson = require('./build/contracts/MATICSmashnado.json')
  ...
  if (!(netId === 137 || netId === 80001)) {
    throw new Error('You did not connect to Matic network. Please make sure you connected to Matic Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to Matic Mainnet.')
  erc20 = currency !== 'matic' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  if (inBrowser) {
    const instance = { currency: 'matic', amount: '1' }
    //await init(instance)
    ...
    program
      ...
      .description('Submit a deposit of specified currency and amount from default eth account and return the resulting note. The currency is one of (ETH|DAI|cDAI|USDC|cUSDC|USDT).' )
